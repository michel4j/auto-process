...
[f] Percentage correlation between intensities from random half-datasets. (see Karplus & Diederichs (2012), Science. 336 (6084): 1030-1033)""" % \
                        resol_method[_summary['summary']['resolution'][1]]
...
[c] %s \n""" % dres['strategy'].get('resolution_reasoning', 'N/A')
        ...
        [!] Space Group is ambiguous. The lowest symmetry group of the high probability candidates has been chosen to permit reindexing in the future without loss of data!"""
            ...
            _section['plot_axes'] = [('Frame No.', ['Scale Factor']), ('Frame No.', ['Mosaicity']),
                                     ('Frame No.', ['Divergence'])]
            ...
            if dres.get('scaling'):
                ...
                _section['plot_axes'] = [('Resolution', ['R_meas', 'CC(1/2)', 'Completeness']),
                                         ('Resolution', ['I/Sigma'])]
            ...
            for row in dres['correction']['statistics']:
                n_row = SortedDict()
                for k, t in [('Resolution', 'shell'), ('Completeness', 'completeness'), ('R_meas', 'r_meas'),
                             ('CC(1/2)', 'cc_half'), ('I/Sigma', 'i_sigma'), ('SigAno', 'sig_ano'),
                             ('AnoCorr', 'cor_ano')]:
                    n_row[k] = row[t]
        ...
        if dres.get('scaling'):
            ...
            _section['shells']['plot_axes'] = [('Resolution', ['R_meas', 'CC(1/2)', 'Completeness']),
                                               ('Resolution', ['I/Sigma'])]
            if options.get('anomalous', False):
...
def get_reports(datasets, options={}):
    ...
    for _name, dset in datasets.items():
        ...
        _sum_values = [
            ...
            dres['correction']['correction_factors']['parameters'][0].get('ISa', -1)
        ]
        ...
        if options.get('mode', None) == 'screen':
            ...
            if dres.get('strategy') and dres['strategy'].get('runs'):
                ...
                _res_shells = [(x + y) / 2.0 for x, y in
                               zip(_st_shell.get('min_resolution', []), _st_shell.get('max_resolution', []))]
...
def save_log(info, filename):
    ...
    for name, dset in info['details'].items():
        ...
        if dset.get('symmetry') is not None:
            ...
            file_text += format_section(dset['symmetry']['lattices'], level=3,
                                        invert=True, fields=['No.', 'Cell Volume', 'Reindexing Matrix'],
                                        show_title=False)

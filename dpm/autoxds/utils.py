...
def save_files(prefix):
    ...
    shutil.move('CORRECT.LP','%s-CORRECT.LP' % prefix)
    shutil.move('INTEGRATE.LP','%s-INTEGRATE.LP' % prefix)
    shutil.move('XDS_ASCII.HKL','%s-XDS_ASCII.HKL' % prefix)
    shutil.move('INTEGRATE.HKL','%s-INTEGRATE.HKL' % prefix)
    shutil.move('GXPARM.XDS','%s-GXPARM.XDS' % prefix)
    files = {
...
def score_crystal(resolution, mosaicity, r_meas, std_spot, std_spindle, subtree_skew, ice_rings):
    score = 1.0
    score -= 0.7 * math.exp(-4.0 / resolution) # subtracts 0.2 for limit resolution of 3.2A
    score -= 0.1 * std_spindle 
    score -= 0.05 * std_spot 
    score -= 0.2 * mosaicity
    score -= 0.01 * r_meas
    #score -= 0.1 * ice_rings

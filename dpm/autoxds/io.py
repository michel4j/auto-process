...
#        _file_template = os.path.join(tmp_dir, xds_template)
    ...
    _num_cpus = int(round(DEFAULT_DELPHI / params['delta_angle']))
    _num_cpus = min(_num_cpus, params['cpu_count'])
    
    #determine max number of jobs cores available and _num_cpus
    _frames = params['data_range'][1] - params['data_range'][0] + 1
    _batches = _frames // _num_cpus
 
    _jobs = _batches // (int(os.environ.get('DPM_CORES', 2)) // _batches)

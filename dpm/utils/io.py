...
def get_job_params(num_frames, delta, delphi=DEFAULT_DELPHI):
    ...
    batch_size = ceiling(delphi / delta, 4)
...
def write_xds_input(jobs, params):
    ...
    _default_delphi = 1.0 if params.get('mode') == 'screen' else DEFAULT_DELPHI
    _frames = params['data_range'][1] - params['data_range'][0] + 1
    if params.get('mode') != 'screen':
        _jobs, batch_size, _num_cpus = get_job_params(_frames, params['delta_angle'], delphi=_default_delphi)
        _delphi = batch_size * params['delta_angle']
    else:
        _jobs, batch_size, _num_cpus = get_job_params(_frames, params['delta_angle'], delphi=_default_delphi)
        _delphi = _default_delphi
    ...
    file_text += "BACKGROUND_RANGE=%d %d \n" % params.get('background_range', (
    params['data_range'][0], max(params['data_range'][1] // 10, 10)))

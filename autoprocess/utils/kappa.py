...
def main(GoniometerAxes, XOparser, mode, v1, v2, datum, pg_permute=True):
    ...
    Goniometer.setAngles(list(map(degree2radian, setDatum)))
...
def get_solutions(info, orientations=("", ""), mode='MAIN'):
    from XO.XOalign_sitedef import GONIOMETER_AXES, GONIOMETER_DATUM, GONIOMETER_NAME

    _do_PG_permutations = True

    # Default orientation
    _v1, _v2 = list(map(str, orientations))
    ...
    return list(independent_solutions.values()), {'mode': mode, 'goniometer': GONIOMETER_NAME}
